...
namespace ConditionFramework
{
    ...
    {
        //constructor
        public MarkovGenerator(Random inRnd, IEnumerable<string> sampleNames, int order, int minLength)
        {
            // pass in the global randomizer
            _rnd = inRnd;
            ...
            {
                for (int currletteridx = 0; currletteridx < word.Length - order; currletteridx++)
                {
                    string token = word.Substring(currletteridx, order);
                    ...
                    entry.Add(word[currletteridx + order]);
                }
            }
